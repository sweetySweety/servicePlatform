  ...
  <div>
    ...
    >
      <a-tab-pane v-for="pane in panes" :tab="pane.title" :key="pane.key" :closable="pane.key == '1'?false : true" style="overflow-y:scroll; maxHeight:'100px" >
        <!-- {{content}} -->
        <todoList v-if="pane.key =='1'" @goInfo="goInfo"></todoList>
        <viewInfo v-else-if="pane.key == '/myworkspace/todolist/viewInfo'"></viewInfo>
      </a-tab-pane>
    </a-tabs>
  </div>
</template>
<script>
  import todoList from './todolistManage'
  import viewInfo from '../myworkspace/todoList/viewInfo'
  export default {
    data () {
      const panes = [
        { title: '待办事宜', content: '', key: '1' }
      ]
    ...
    methods: {
      ...
      goInfo(){
        ...
        const activeKey = `/myworkspace/todolist/viewInfo`
